...
def swap_tokens(args, x_embeds, max_len, cos_ids, lm, model, true_labels, true_grads, counts_swap ):
        ...
        for sample_idx in range(200):
            ...
            if sample_idx != 0:
                if sample_idx % 4 == 0: # swap two tokens
                    ...
                    # indices = np.random.choice(np.arange(1, max_len[sen_id] - 1), size=3, replace=False)
                    # x, y, z = sorted(indices)
                    # perm_ids = np.concatenate(
                    #     [perm_ids[:x], perm_ids[y + 1:z + 1], perm_ids[y:y + 1], perm_ids[x:y], perm_ids[z + 1:]])
                    i = 1 + np.random.randint(max_len[sen_id] - 2)
...
def reconstruct(args, device, sample, metric, tokenizer, lm, model, counts_swap):
    ...
    for it in range(args.n_steps):
        ...
        if args.use_swaps and it >= args.swap_burnin * args.n_steps and it % args.swap_every == 1:
            ...
            swap_tokens(args, x_embeds, max_len, cos_ids, lm, model, true_labels, true_grads, counts_swap)
            print('prediction: %s' % (tokenizer.batch_decode(cos_ids)), flush=True)
            print(counts_swap)
